...
// hàm cập nhật chung (title, description, type)
...
const pushMemberIds = async (boardId, memberId) => {
  try {
    return await GET_DB()
      ...
      .findOneAndUpdate(
        { _id: new ObjectId(String(boardId)) },
        { $push: { memberIds: new ObjectId(String(memberId)) } },
        { returnDocument: 'after' },
      )
  } catch (error) {
    throw error
...
const pullMemberIds = async (boardId, memberId) => {
  try {
    return await GET_DB()
      .collection(BOARD_COLLECTION_NAME)
      .findOneAndUpdate(
        { _id: new ObjectId(String(boardId)) },
        { $pull: { memberIds: new ObjectId(String(memberId)) } },
        { returnDocument: 'after' },
      )
  } catch (error) {
    throw error
